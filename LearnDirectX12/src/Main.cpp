	...
	const wchar_t* windowTitle, uint32_t width, uint32_t height )
{
	...
	// Dimensions of the adjusted window rectangle 
	// are used to compute the width and height
	// of the window that is to be created. 
...
#if defined(_DEBUG)
	createFactoryFlags = DXGI_CREATE_FACTORY_DEBUG; // Should not be used in production builds. 
#endif
...
{
	auto rtvDescriptionSize = device->GetDescriptorHandleIncrementSize( D3D12_DESCRIPTOR_HEAP_TYPE_RTV );
	...
	for ( int i = 0; i < g_NumFrames; ++i )
	{
		...
		rtvHandle.Offset( rtvDescriptionSize );
	}
}
...
ComPtr<ID3D12Fence> CreateFence( ComPtr<ID3D12Device2> device )
{
	...
	ThrowIfFailed( device->CreateFence( 0, // Initial value should be 0.  
...
void Render()
{
	// Pointers to the command allocator and back buffer resource
	// are retrieved according to the current back buffer index.
	auto commandAllocator = g_CommandAllocators[g_CurrentBackBufferIndex];
	auto backBuffer = g_BackBuffers[g_CurrentBackBufferIndex];

	// The command allocator and command list are reset.
	// This prepares the command list for recording the next frame.
...
void SetFullscreen( bool fullscreen )
{
	if ( g_Fullscreen != fullscreen )
	{
		...
		if ( g_Fullscreen ) // Switching to fullscreen.
		{
			...
			// when swtiching out of fullscreen state.
