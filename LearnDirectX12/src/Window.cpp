...
Window::Window( HWND hWnd, const std::string& windowName, int clientWidth, int clientHeight, bool vSync )
	:
	m_Window( hWnd ),
...
Window::~Window()
{
	...
	assert( !m_Window && "Use Application::DestroyWindow before destruction." );
}
...
void Window::Destroy()
{
	...
	// Remove back buffer resource from the global resource tracker.
...
Microsoft::WRL::ComPtr<IDXGISwapChain4> Window::CreateSwapChain()
{
	...
	ThrowIfFailed( dxgiFactory4->MakeWindowAssociation( m_Window, DXGI_MWA_NO_ALT_ENTER ) );
...
unsigned int Window::GetCurrentBackBufferIndex() const
{
	return m_CurrentBackBufferIndex;
}

unsigned int Window::Present()
{
	unsigned int syncInterval = m_VSync ? 1 : 0;
	unsigned int presentFlags = m_IsTearingSupported && !m_VSync ? DXGI_PRESENT_ALLOW_TEARING : 0;
